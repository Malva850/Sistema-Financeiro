...
type
  ...
  public
    constructor create;
    function Inserir(var PCodigo : Integer;  PCdLancamento : string; var PMensagem:String): Boolean;
    ...
    function InserirPorBaixaLancamento(var PCodigo: Integer;
...
uses UDM, System.SysUtils, ULancamentos;
...
function TLancamentosCaixas.Inserir(var PCodigo : Integer; PCdLancamento : string;  var PMensagem:String): Boolean;
var
   lanc: Lancamentos;
begin
  try
   ...
   // dm.QInsere.ParamByName('PHR_LANCAMENTO').AsString :=  FHrLancamento;
    ...
    if CdLancamentoCaixa = 0 then
      ...
      dm.QInsere.ParamByName('PCD_LANCAMENTO_CAIXA_ESTORNO').AsInteger := CdLancamentoCaixaEstorno;
    ...
    if PCdLancamento<>'' then
    begin
       lanc := Lancamentos.Create;
       lanc.Baixar2(FCdLancamentoCaixa,  strtoint(PCdLancamento),  FVlLancamento);
       lanc.Free;
    end;
    ...
    begin
      ...
      dm.QUpdate.ParamByName('PCD_LANCAMENTO_CAIXA').AsInteger := CdLancamentoCaixaEstorno;
      dm.QUpdate.ExecSQL;


      dm.QUpdate.Close;
      dm.QUpdate.SQL.Clear;
      dm.QUpdate.SQL.Add(' UPDATE LANCAMENTOS SET DT_PAGAMENTO=NULL , VL_PAGAMENTO=0, CD_LANCAMENTO_CAIXA=NULL  WHERE CD_LANCAMENTO_CAIXA=:PCD_LANCAMENTO_CAIXA');
      dm.QUpdate.ParamByName('PCD_LANCAMENTO_CAIXA').AsInteger := CdLancamentoCaixaEstorno;
      dm.QUpdate.ExecSQL;
...
begin
  Query.Close;
  Query.SQL.Clear;
  Query.SQL.Add(' select  lc.cd_caixa, LC.cd_lancamento_caixa,  c.nm_subcategoria, lc.vl_lancamento , lc.CD_LANCAMENTO_CAIXA_ESTORNO'+
                ' from lancamentos_caixas lc, subcategorias c '+
                ' where  lc.cd_subcategoria=c.cd_subcategoria'+
                ' and lc.cd_abertura_caixa = :pcd_abertura_caixa '+
...
function TLancamentosCaixas.InserirPorBaixaLancamento(var PCodigo: Integer;
  var PMensagem: string): Boolean;
begin
   try
    ...
    dm.QInsere.ParamByName('PDS_LANCAMENTO').AsString := FDsLancamento;
    dm.QInsere.ParamByName('PCD_LANCAMENTO_CAIXA_ESTORNO').Clear();
    dm.QInsere.ParamByName('PFL_ESTORNO').Clear();
...
begin
  ...
  begin
    ...
    if dm.QVaziaDM.FieldByName('FL_ESTORNO').AsString = 'S' then
    begin
      Result := false;
      EXIT;
    end;
