...
begin
  try
    ...
    dm.QInsere.SQL.Add(' INSERT INTO PESSOAS (CD_PESSOA, NM_PESSOA, NR_CPF_CNPJ, NR_CELULAR)'+
                       ' VALUES (:PCD_PESSOA, :PNM_PESSOA, :NR_CPF_CNPJ , :PNR_CELULAR)');
    dm.QInsere.ParamByName('PCD_PESSOA').AsInteger := FCdPessoa ;
    dm.QInsere.ParamByName('PNM_PESSOA').AsString  := FNmPessoa;
    dm.QInsere.ParamByName('NR_CPF_CNPJ').AsString := Fnr_cpf_cnpj;
...
begin
  try
    ...
    dm.QUpdate.SQL.Add(' UPDATE PESSOAS SET NM_PESSOA=:PNM_PESSOA, NR_CPF_CNPJ=:NR_CPF_CNPJ, NR_CELULAR=:PNR_CELULAR WHERE CD_PESSOA=:PCD_PESSOA');
    dm.QUpdate.ParamByName('PNM_PESSOA').AsString  := FNmPessoa;
    dm.QUpdate.ParamByName('PCD_PESSOA').AsInteger := FCdPessoa ;
    dm.QUpdate.ParamByName('NR_CPF_CNPJ').AsString := Fnr_cpf_cnpj;
    dm.QUpdate.ParamByName('PNR_CELULAR').AsString  := FNrCelular;
    dm.QUpdate.ExecSQL;
...
function TPessoas.Localizar(PCodigo, PNome : String; var Query : TFDQuery): boolean;
begin
  Query.Close;
  Query.SQL.Clear;
