...
public
   constructor create;
    ...
    function RetornaCodigoUsuario(Login: String): integer;
...
function TUsuario.VerificarLogin(login, senha: string): Boolean;
var
...
begin
  ...
  begin
    ...
    UsuarioLogado := DM.QSelect.FieldByName('LOGIN').AsString ;
    IdUsuarioLogado := DM.QSelect.FieldByName('CD_USUARIO').AsInteger ;
...
Query.Close;
  Query.SQL.Clear;
  Query.SQL.Add(' select * from USUARIOS WHERE 1=1');
...
function TUsuario.RetornaCodigoUsuario(Login: String): integer;
begin
  dm.QVaziaDM.Close;
  dm.QVaziaDM.SQL.Clear;
  dm.QVaziaDM.SQL.Add(' select cd_usuario from USUARIOS WHERE login=:plogin');
  dm.QVaziaDM.ParamByName('plogin').AsString := Login ;
  dm.QVaziaDM.Open();
  if not dm.QVaziaDM.IsEmpty then
  begin
    Result := dm.QVaziaDM.FieldByName('cd_usuario').AsInteger;
  end
  else
  begin
    Result := 0;
