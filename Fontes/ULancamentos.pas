...
type
  ...
  public
    ...
    function Localizar(dtVencInicial, dtVencFinal, dtPagInicial, dtPagFinal, tipoPagamento: string; var Query : TFDQuery): boolean;
    function LocalizarPorSubcategoria(cdSubcategoria: Integer;
      var Query: TFDQuery): Boolean;
    function Baixar(cdLancamentoCaixa, cdLancamento: Integer): Boolean;
    function Baixar2(cdLancamentoCaixa, cdLancamento: Integer; PValor: extended): Boolean;
...
function Lancamentos.Baixar(cdLancamentoCaixa, cdLancamento: Integer): Boolean;
begin
  dm.DBFinanceiro.StartTransaction;
  dm.QVaziaDM.Close;
  dm.QVaziaDM.SQL.Clear;
  dm.QVaziaDM.SQL.Add(' UPDATE LANCAMENTOS SET DT_PAGAMENTO = CURRENT_DATE, ' +
                      ' VL_PAGAMENTO = VL_LANCAMENTO, CD_LANCAMENTO_CAIXA =:PCD_LANCAMENTO_CAIXA ' +
                      ' WHERE CD_LANCAMENTO =:PCD_LANCAMENTO ');

  dm.QVaziaDM.ParamByName('PCD_LANCAMENTO_CAIXA').AsInteger := cdLancamentoCaixa;
  dm.QVaziaDM.ParamByName('PCD_LANCAMENTO').AsInteger := cdLancamento;
...
function Lancamentos.Baixar2(cdLancamentoCaixa, cdLancamento: Integer; PValor: extended): Boolean;
begin
  //dm.DBFinanceiro.StartTransaction;
  dm.QVaziaDM.Close;
  dm.QVaziaDM.SQL.Clear;
  dm.QVaziaDM.SQL.Add(' UPDATE LANCAMENTOS SET DT_PAGAMENTO = CURRENT_DATE, ' +
                      ' VL_PAGAMENTO = :VL_PAGAMENTO , CD_LANCAMENTO_CAIXA =:PCD_LANCAMENTO_CAIXA ' +
                      ' WHERE CD_LANCAMENTO =:PCD_LANCAMENTO ');

  dm.QVaziaDM.ParamByName('PCD_LANCAMENTO_CAIXA').AsInteger := cdLancamentoCaixa;
  dm.QVaziaDM.ParamByName('PCD_LANCAMENTO').AsInteger := cdLancamento;
  dm.QVaziaDM.ParamByName('VL_PAGAMENTO').AsFloat := PValor;
...
begin
  try
    ...
    dm.QUpdate.ParamByName('VL_PAGAMENTO').AsExtended := FVlPagamento;
     if FCdLancamentoCaixa=0 then
    begin
      dm.QUpdate.ParamByName('CD_LANCAMENTO_CAIXA').Clear;
    end
    else
    begin
...
begin
   try
    ...
    dm.DBFinanceiro.StartTransaction;
    FCdLancamento := dm.GeraSequencial('GEN_LANCAMENTOS') ;
    ...
    if FDtPagamento=strtodate('11/11/1111') then
    begin
      dm.QInsere.ParamByName('DT_PAGAMENTO').Clear;
    end
    else
    begin
      ...
      dm.QInsere.ParamByName('VL_PAGAMENTO').AsExtended := FVlPagamento;

    if FCdLancamentoCaixa=0 then
    begin
      dm.QInsere.ParamByName('CD_LANCAMENTO_CAIXA').Clear;
    end
    else
    begin
...
function Lancamentos.Localizar(dtVencInicial, dtVencFinal, dtPagInicial,
  dtPagFinal, tipoPagamento: string; var Query: TFDQuery): boolean;
begin
  Query.Close;
  Query.SQL.Clear;
  Query.SQL.Add(' SELECT * FROM LANCAMENTOS AS LANC ' +
                ' INNER JOIN EMPRESAS AS EMP ON LANC.CD_EMPRESA = EMP.CD_EMPRESA ' +
                ' INNER JOIN TIPOS_PAGAMENTOS AS PAG ' +
                ' ON LANC.CD_TIPO_PAGAMENTO = PAG.CD_TIPO_PAGAMENTO WHERE 1=1');

    if Trim(dtVencInicial) <> '' then
  begin
    //dtVencInicial := dtVencInicial.Replace('/', '.');
    Query.SQL.Add(' AND LANC.DT_VENCIMENTO >=:DT_INICIAL_VENC');
    Query.ParamByName('DT_INICIAL_VENC').AsDate := StrToDate(dtVencInicial);
  end;

  if Trim(dtVencFinal) <> '' then
  begin
    dtVencFinal := dtVencFinal.Replace('/', '.');
    Query.SQL.Add(' AND LANC.DT_VENCIMENTO <=:DT_FINAL_VENC');
    Query.ParamByName('DT_FINAL_VENC').AsString := dtVencFinal;
  end;

  if Trim(dtPagInicial) <> '' then
  begin
    dtPagInicial := dtPagInicial.Replace('/', '.');
    Query.SQL.Add(' AND LANC.DT_PAGAMENTO >=:DT_INICIAL_PAG');
    Query.ParamByName('DT_INICIAL_PAG').AsString := dtPagInicial;
  end;

  if Trim(dtPagFinal) <> '' then
  begin
    dtPagFinal := dtPagFinal.Replace('/', '.');
    Query.SQL.Add(' AND LANC.DT_PAGAMENTO <=:DT_FINAL_PAG');
    Query.ParamByName('DT_FINAL_PAG').AsString := dtPagFinal;
  end;

  if Trim(tipoPagamento) <> '' then
  begin
    Query.SQL.Add(' AND PAG.CD_TIPO_PAGAMENTO =:TIPO_PAGAMENTO');
    Query.ParamByName('TIPO_PAGAMENTO').AsString := tipoPagamento;
  end;
...
function Lancamentos.LocalizarPorSubcategoria(cdSubcategoria: Integer;
  var Query: TFDQuery): Boolean;
begin
Query.Close;
  Query.SQL.Clear;
  Query.SQL.Add(' SELECT * FROM LANCAMENTOS AS LANC ' +
                ' INNER JOIN EMPRESAS AS EMP ON LANC.CD_EMPRESA = EMP.CD_EMPRESA ' +
                ' INNER JOIN TIPOS_PAGAMENTOS AS PAG ' +
                ' ON LANC.CD_TIPO_PAGAMENTO = PAG.CD_TIPO_PAGAMENTO ' +
                ' WHERE LANC.CD_SUBCATEGORIA = :PCD_SUBCATEGORIA ' +
                ' AND LANC.CD_LANCAMENTO_CAIXA IS NULL');

  Query.ParamByName('PCD_SUBCATEGORIA').AsInteger := cdSubcategoria;
